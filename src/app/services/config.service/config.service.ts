...
import { IConfig, IAuthenticationConfig, InitializeConfig, ILogging } from 'src/environments/config/config.interface';
...
export class ServiceConfig implements IConfig {
    ...
    */
    ...
    loadConfig(): Promise<void> {
        return this.http.get<IConfig>(AppSettings.configFileLocation).toPromise().then((config: IConfig) => {
            ...
            switch (config.logging.loggingLevel) {
                ...
                case 'error':
                    ...
                    break;
                default:
                    console.warn('Loggin level not set in config.  Level set to \'Info\' by default');
                    this.logging.loggingLevel = Enums.ErrorSeverityLevel.Info;
                    break;
            }
